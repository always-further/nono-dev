...
def add_parser(subparsers):
    ...
    parser.add_argument(
        "pr_number", type=int, help="GitHub pull request number to review",
    )
...
def run(args):
    ...
    session_name = f"review-{args.pr_number}"
    ...
    session_id = nono.run_detached(
        ...
        user_prompt=str(args.pr_number),
        rollback=rollback,
    )

    print(f"Review session started for PR #{args.pr_number} ({repo})")
